# app/services/vector_store_cloud.py
import chromadb
...
class CloudVectorStore:
    """ChromaDB vector store with cloud-based embeddings"""
    ...
    def __init__(self):
        try:
            ...
            self.collection = self.client.get_or_create_collection(
                ...
                metadata={"description": "Document chunks for RAG system with cloud embeddings"}
            )
            
            logger.info(f"CloudVectorStore initialized. Collection: {settings.chroma_collection_name}")
            logger.info(f"Current collection size: {self.collection.count()}")
            
        except Exception as e:
            logger.error(f"Error initializing CloudVectorStore: {e}")
            raise
    
    async def add_document_chunks(self, chunks: List[DocumentChunk], document_metadata: Dict = None) -> bool:
        """Add document chunks to the vector store using cloud embeddings"""
        try:
            if not chunks:
            ...
            logger.info(f"Successfully added {len(chunks)} chunks to vector store with cloud embeddings")
            return True
    ...
    async def search_similar_chunks(
        ...
        """Search for similar chunks using cloud-based semantic similarity"""
        try:
            # Generate query embedding using cloud service
            ...
            logger.info(f"Found {len(formatted_results)} similar chunks for query using cloud embeddings")
